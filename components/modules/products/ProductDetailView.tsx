...
function ProductDetailContent({ product }: { product: Product }) {
  ...
  const [activeTab, setActiveTab] = useState(
    searchParams.get('tab') || 'key-benefits',
  );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black pb-28 lg:pb-20">
      ...
      {showQRPopup && (
        <QRAuthPopup product={product} onClose={handleCloseQRPopup} />
      )}
      {/* Breadcrumb */}
      <div className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
        <div className="container-width py-4 flex items-center gap-2 text-xs md:text-sm text-zinc-500 flex-wrap">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-zinc-900 dark:text-zinc-100 font-medium truncate max-w-37.5 md:max-w-xs">
            {product.name}
          </span>
        </div>
      </div>
      ...
      <div className="container-width py-8 lg:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-6 space-y-6">
            <div
              className="relative h-100 lg:h-112.5 w-full bg-zinc-50 dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 flex items-center justify-center p-8 overflow-hidden group"
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide justify-center">
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-16 h-16 rounded-xl border-2 bg-zinc-50 dark:bg-zinc-900 p-2 shrink-0 transition-all ${activeImage === img ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-transparent hover:border-zinc-300 dark:hover:border-zinc-700'}`}
                  >
                    <Image
                      src={img}
                      alt=""
          ...
          <div className="lg:col-span-6 flex flex-col">
            ...
            {product.availablePackSizes &&
              product.availablePackSizes.length > 0 && (
                <div className="mb-8">
                ...
                <div className="mb-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
                    {product.recommendedCrops.map((crop, idx) => (
                      <div
                        key={idx}
                        className="flex flex-col items-center gap-2 shrink-0 snap-start group cursor-default min-w-17.5"
                      >
                        <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-zinc-100 dark:border-zinc-800 group-hover:border-emerald-500 transition-colors shadow-sm">
                          <Image
                            src={
                              crop.image ||
                              '/images/farmerlift_icon_transparent.png'
                            }
                            alt={crop.name}
                            fill
        ...
        <div className="mt-12 lg:mt-16">
          ...
          >
              ...
              {[
                ...
                return (
                  <TabsTrigger
                    key={val}
                    value={val}
                    className="flex-1 min-w-35 bg-transparent border-b-2 border-transparent data-[state=active]:border-emerald-600 data-[state=active]:text-emerald-700 data-[state=active]:shadow-none rounded-none px-2 py-3 text-sm sm:text-base font-medium text-zinc-500 hover:text-zinc-700 whitespace-nowrap"
                  >
                    {tab}
                  </TabsTrigger>
                );
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {product.features.map((feature, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-4 p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-colors"
                    >
                      <div className="mt-1 h-6 w-6 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center shrink-0">
            ...
            >
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 md:p-8">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                  <Info className="h-5 w-5 text-emerald-600" /> Technical
                  Specifications
                </h3>
                  ...
                  <div className="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
                    <table className="w-full text-sm text-left">
                      <thead className="bg-zinc-50 dark:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300 font-bold uppercase tracking-wider">
                        <tr>
                          <th className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 w-2/3">
                            Specifications
                          </th>
                          <th className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 w-1/3 text-right">
                            Value / %
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                        {product.detailedSpecs.map((spec, i) => (
                          ...
                          >
                            <td className="px-6 py-4 font-medium text-zinc-600 dark:text-zinc-400 border-r border-zinc-100 dark:border-zinc-800">
                              {spec.key}
                            </td>
                            <td className="px-6 py-4 font-bold text-em dark:text-emerald-400 text-right text-emerald-600 ">
                              {spec.value}
                            </td>
                          </tr>
                        ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2">
                    ...
                    {product.category === 'fertilizer' && (
                      <>
                        <SpecRow
                          label="Type"
                          value={
                            (product.specifications as FertilizerSpecs).type
                          }
                        />
                        <SpecRow
                          label="Form"
                          value={
                            (product.specifications as FertilizerSpecs).form
                          }
                        />
                        <SpecRow
                          label="NPK Ratio"
                          value={
                            (product.specifications as FertilizerSpecs).npkRatio
                          }
                        />
                        <SpecRow
                          label="Solubility"
                          value={
                            (product.specifications as FertilizerSpecs)
                              .solubility
                          }
                        />
                      </>
                    ...
                    {product.category === 'pesticide' && (
                      <>
                        <SpecRow
                          ...
                          value={
                            (product.specifications as PesticideSpecs)
                              .chemicalGroup
                          }
                        />
                        <SpecRow
                        ...
                        <SpecRow
                          ...
                          value={
                            (product.specifications as PesticideSpecs)
                              .activeIngredients[0]?.formulation
                          }
                        />
                      </>
                    )}
                    {product.category === 'seed' && (
                      <>
                        <SpecRow
                          label="Variety"
                          value={(product.specifications as SeedSpecs).variety}
                        />
                        <SpecRow
                          label="Duration"
                          value={(product.specifications as SeedSpecs).duration}
                        />
                        <SpecRow
                          label="Season"
                          value={(
                            product.specifications as SeedSpecs
                          ).season?.join(', ')}
                        />
                      </>
                    )}
                    {product.category === 'machinery' && (
                      <>
                        <SpecRow
                          label="Power Source"
                          value={
                            (product.specifications as MachinerySpecs)
                              .powerSource
                          }
                        />
                        <SpecRow
                          label="Capacity"
                          value={
                            (product.specifications as MachinerySpecs).capacity
                          }
                        />
                        <SpecRow
                          label="Warranty"
                          value={
                            (product.specifications as MachinerySpecs).warranty
                          }
                        />
                      </>
                    )}
            ...
            <TabsContent
              value="method-of-application"
              className="animate-in fade-in slide-in-from-bottom-4 duration-500"
            >
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 md:p-8">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                  <Leaf className="h-5 w-5 text-emerald-600" /> Method of
                  Application
                </h3>
                ...
                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 border-b border-zinc-200 dark:border-zinc-700 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="text-center md:text-left">
                    ...
                    <p className="text-sm text-zinc-500 dark:text-zinc-400">
                      Official technical specifications & regulatory compliance
                    </p>
                  </div>
                  <div className="bg-white p-2 rounded-lg border border-zinc-100 shadow-sm shrink-0">
                    {product.wordpressId ? (
                      ...
                      <div className="w-20 h-20 bg-zinc-100 flex items-center justify-center rounded text-[10px] text-zinc-400">
                        NO ID
                      </div>
                    )}
                  </div>
                ...
                <div className="p-6 md:p-8 space-y-8">
                  <div className="text-center pb-6 border-b border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="inline-block px-4 py-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full border border-zinc-200 dark:border-zinc-700">
                      <span className="text-[10px] font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">
                        Order:{' '}
                        {product.qrTabDetails?.gazetteNumber ||
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div className="bg-zinc-50 dark:bg-zinc-800/30 rounded-xl p-4 sm:p-5 border border-zinc-100 dark:border-zinc-800">
                        <FormattedText
                          text={
                            product.qrTabDetails?.composition ||
                            'Composition details are mentioned on the physical packaging.'
                          }
                    ...
                    <div className="space-y-6">
                      <div>
                        <h5 className="font-bold text-sm text-zinc-900 dark:text-white flex items-center gap-2 mb-3">
                          ...
                          Approved Target Crops
                        </h5>
                        <p className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">
                          {product.qrTabDetails?.crops ||
                            'All major field, fruit, and vegetable crops.'}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <FormattedText
                          text={
                            product.qrTabDetails?.dosage ||
                            'Refer to the method of application tab or packaging.'
                          }
                  ...
                  <div className="mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-800 text-center">
                    <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-2xl mx-auto">
                      This electronic record is generated for the purpose of
                      product identification and authenticity verification under
                      the guidelines of FarmerLift Quality Assurance. For
                      batch-specific details (MFG, EXP, Batch No.), please refer
                      to the printed label on the container.
                    </p>
                    <div className="mt-4 flex items-center justify-center gap-4">
                      <div className="flex items-center gap-1.5">
                        <ShieldCheck className="h-4 w-4 text-emerald-600" />
                        <span className="text-[10px] font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">
                          Verified Authentic
                        </span>
                      </div>
