...
const ResourceCard: React.FC<ResourceCardProps> = ({ type, item }) => {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
      {/* Media Placeholder */}
      <div className={`aspect-video relative ${isBlog ? 'bg-gray-100' : 'bg-gray-900'} flex items-center justify-center`}>
         {isBlog ? (
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex items-center justify-between mb-3">
          <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
            isBlog ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
          }`}>
            {type}
          </span>
          {isBlog && (
        ...
        <div className="mt-auto">
           <Link href={isBlog ? `/resources/blog/${item.id}` : `/resources/videos`} className="block">
